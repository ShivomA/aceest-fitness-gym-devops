...
class FitnessTrackerApp:
    ...
    def create_log_tab(self):
        # Header
        tk.Label(self.log_tab, text="ACEest Fitness & Gym Tracker", font=("Helvetica", 18, "bold"), bg="#f0f0f0",
                 fg="#343a40").pack(pady=15)
        ...
        tk.Label(input_container, text="Select Category:", font=("Arial", 12, "bold"), bg="#e9ecef").grid(row=0,
                                                                                                          column=0,
                                                                                                          sticky="w",
                                                                                                          padx=10,
                                                                                                          pady=5)
        self.category_menu = ttk.Combobox(input_container, textvariable=self.category_var,
                                          values=list(self.workouts.keys()), state="readonly", width=20,
                                          font=("Arial", 11))
        self.category_menu.grid(row=0, column=1, sticky="w", padx=10, pady=5)

        # Exercise Input
        tk.Label(input_container, text="Exercise:", font=("Arial", 12, "bold"), bg="#e9ecef").grid(row=1, column=0,
                                                                                                   sticky="w", padx=10,
                                                                                                   pady=5)
        ...
        # Duration Input
        tk.Label(input_container, text="Duration (min):", font=("Arial", 12, "bold"), bg="#e9ecef").grid(row=2,
                                                                                                         column=0,
                                                                                                         sticky="w",
                                                                                                         padx=10,
                                                                                                         pady=5)
        ...
        ttk.Button(button_frame, text="✅ Add Session", command=self.add_workout, width=20, style="Add.TButton").grid(
            row=0, column=0, padx=15)
        ttk.Button(button_frame, text="📋 View Summary", command=self.view_summary, width=20,
                   style="Summary.TButton").grid(row=0, column=1, padx=15)

        # Status Bar
        self.status_label = tk.Label(self.log_tab, text="Welcome! Log your first session.", bd=1, relief=tk.SUNKEN,
                                     anchor=tk.W, bg="#fff", fg="#6c757d", font=("Arial", 10))
    ...
    def view_summary(self):
        ...
        tk.Label(summary_window, text="📊 Weekly Session Summary", font=("Helvetica", 16, "bold"), bg="#f8f9fa",
                 fg="#343a40").pack(pady=10)

        # Use a scrolled text area for better presentation of many entries
        summary_text = tk.Text(summary_window, height=20, width=55, wrap=tk.WORD, font=("Arial", 10), bg="white",
                               fg="#343a40")
        ...
        for category, sessions in self.workouts.items():
            ...
            summary_text.tag_config(category.lower(), font=("Arial", 12, "bold"),
                                    foreground="#007bff" if category == "Warm-up" else "#28a745" if category == "Workout" else "#ffc107")
    ...
    def create_workout_plan_tab(self):
        tk.Label(self.chart_tab, text="💡 Personalized Workout Plan", font=("Helvetica", 18, "bold"), bg="#f0f0f0",
                 fg="#343a40").pack(pady=15)
        ...
        chart_data = {
            "Warm-up (5-10 min)": ["5 min light cardio (Jog/Cycle)", "Jumping Jacks (30 reps)",
                                   "Arm Circles (15 Fwd/Bwd)"],
            "Strength Workout (45-60 min)": ["Push-ups (3 sets of 10-15)", "Squats (3 sets of 15-20)",
                                             "Plank (3 sets of 60 seconds)", "Lunges (3 sets of 10/leg)"],
        ...
        for category, exercises in chart_data.items():
            tk.Label(plan_frame, text=f"🔥 {category}", font=("Arial", 14, "bold"), bg="white", fg="#007bff").pack(
                anchor="w", padx=10, pady=(10, 5))
            for ex in exercises:
                tk.Label(plan_frame, text=f"   • {ex}", font=("Arial", 12), bg="white", fg="#495057").pack(anchor="w",
                                                                                                           padx=30)

    # ------------------ DIET GUIDE TAB ------------------ #
    def create_diet_guide_tab(self):
        tk.Label(self.diet_tab, text="🥗 Best Diet Guide for Fitness Goals", font=("Helvetica", 18, "bold"),
                 bg="#f0f0f0", fg="#343a40").pack(pady=15)

        diet_frame = tk.Frame(self.diet_tab, bg="white", padx=20, pady=10, relief=tk.FLAT, bd=1)
        diet_frame.pack(pady=10, padx=50, fill="x")

        diet_plans = {
            "🎯 Weight Loss": ["Breakfast: Oatmeal with Berries", "Lunch: Grilled Chicken/Tofu Salad",
                              "Dinner: Vegetable Soup with Lentils"],
            "💪 Muscle Gain": ["Breakfast: 3 Egg Omelet, Spinach, Whole-wheat Toast",
                              "Lunch: Chicken Breast, Quinoa, and Steamed Veggies",
                              "Post-Workout: Protein Shake, Greek Yogurt"],
            "🏃 Endurance Focus": ["Pre-Workout: Banana & Peanut Butter", "Lunch: Whole Grain Pasta with Light Sauce",
                                  "Dinner: Salmon & Avocado Salad"]
        }

        for goal, foods in diet_plans.items():
            tk.Label(diet_frame, text=f"{goal} Plan:", font=("Arial", 14, "bold"), bg="white", fg="#28a745").pack(
                anchor="w", padx=10, pady=(10, 5))
            for food in foods:
                tk.Label(diet_frame, text=f"   • {food}", font=("Arial", 12), bg="white", fg="#495057").pack(anchor="w",
                                                                                                             padx=30)

    # ------------------ PROGRESS TRACKER TAB ------------------ #
    def create_progress_tab(self):
        tk.Label(self.progress_tab, text="📈 Personal Progress Tracker (Minutes Logged)", font=("Helvetica", 18, "bold"),
                 bg="white", fg="#343a40").pack(pady=15)
    ...
    def update_progress_charts(self):
        ...
        if sum(values) == 0:
            tk.Label(self.chart_container, text="No workout data logged yet. Log a session to see your progress!",
                     font=("Arial", 12, "italic"), fg="#888", bg="white").pack(pady=50)
            return
        ...
        tk.Label(self.progress_tab, text=summary_text, font=("Arial", 12, "bold"), bg="white", fg="#dc3545").pack(
            pady=10)
