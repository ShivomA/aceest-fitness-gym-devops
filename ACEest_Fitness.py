...
class FitnessTrackerApp:
    ...
    def create_log_tab(self):
        tk.Label(self.log_tab, text="🏋️ ACEest Fitness & Gym Tracker", font=("Helvetica", 16, "bold"), bg="white").pack(
            pady=10)

        self.category_var = tk.StringVar(value="Workout")
        tk.Label(self.log_tab, text="Select Category:", font=("Arial", 12), bg="white").pack()
        self.category_menu = ttk.Combobox(self.log_tab, textvariable=self.category_var,
                                          values=list(self.workouts.keys()), state="readonly")
        ...
        tk.Label(input_frame, text="Duration (min):", font=("Arial", 11), bg="white").grid(row=1, column=0, padx=5,
                                                                                           pady=5)
        ...
        tk.Button(button_frame, text="Add Session", command=self.add_workout, width=20, bg="#28a745", fg="white").grid(
            row=0, column=0, padx=5)
        tk.Button(button_frame, text="View Summary", command=self.view_summary, width=20, bg="#007bff",
    ...
    def view_summary(self):
        ...
        for category, sessions in self.workouts.items():
            tk.Label(summary_window, text=f"{category}:", font=("Arial", 12, "bold"), fg="#007bff").pack(anchor="w",
                                                                                                         padx=10)
            if sessions:
                for i, entry in enumerate(sessions, 1):
                    tk.Label(summary_window, text=f" {i}. {entry['exercise']} - {entry['duration']} min",
                             font=("Arial", 11)).pack(anchor="w", padx=25)
                    total_time += entry['duration']
            else:
                tk.Label(summary_window, text="  No sessions recorded.", font=("Arial", 11, "italic"), fg="#888").pack(
                    anchor="w", padx=25)

        tk.Label(summary_window, text=f"\nTotal Time Spent: {total_time} minutes", font=("Arial", 12, "bold"),
                 fg="#28a745").pack(pady=10)

    # ------------------ WORKOUT CHART TAB ------------------ #
    def create_workout_chart_tab(self):
        tk.Label(self.chart_tab, text="🏋️ Personalized Workout Chart", font=("Helvetica", 16, "bold"), bg="white").pack(
            pady=10)
        ...
        for category, exercises in chart_data.items():
            tk.Label(self.chart_tab, text=f"{category} Exercises:", font=("Arial", 13, "bold"), bg="white",
                     fg="#007bff").pack(anchor="w", padx=20, pady=5)
            for ex in exercises:
                tk.Label(self.chart_tab, text=f"• {ex}", font=("Arial", 11), bg="white").pack(anchor="w", padx=40)

    # ------------------ DIET CHART TAB ------------------ #
    def create_diet_chart_tab(self):
        tk.Label(self.diet_tab, text="🥗 Best Diet Chart for Fitness Goals", font=("Helvetica", 16, "bold"),
                 bg="white").pack(pady=10)

        diet_plans = {
            "Weight Loss": ["Oatmeal with Fruits", "Grilled Chicken Salad", "Vegetable Soup", "Brown Rice & Veggies"],
            "Muscle Gain": ["Egg Omelet", "Chicken Breast", "Quinoa & Beans", "Protein Shake",
        ...
        for goal, foods in diet_plans.items():
            tk.Label(self.diet_tab, text=f"{goal} Plan:", font=("Arial", 13, "bold"), bg="white", fg="#28a745").pack(
    ...
    def create_progress_tab(self):
        tk.Label(self.progress_tab, text="📈 Personal Progress Tracker", font=("Helvetica", 16, "bold"),
                 bg="white").pack(pady=10)
