...
class FitnessTrackerApp:
    ...
    def create_log_tab(self):
        # Header
        tk.Label(self.log_tab, text="ACEest Session Logger", font=("Inter", 20, "bold"), bg=COLOR_BACKGROUND,
                 fg=COLOR_TEXT).pack(pady=(20, 10))
        tk.Label(self.log_tab, text="Track your progress with precision.", font=("Inter", 12), bg=COLOR_BACKGROUND,
                 fg="#6C757D").pack(pady=(0, 20))

        # Input Card Container
        log_card = tk.Frame(self.log_tab, bg=COLOR_CARD_BG, padx=40, pady=30, relief=tk.RAISED, bd=0,
        ...
        tk.Label(log_card, text="Category:", font=("Inter", 12, "bold"), bg=COLOR_CARD_BG, fg=COLOR_TEXT).grid(row=0,
                                                                                                               column=0,
                                                                                                               sticky="w",
                                                                                                               padx=10,
                                                                                                               pady=10)
        self.category_menu = ttk.Combobox(log_card, textvariable=self.category_var,
                                          values=list(self.workouts.keys()), state="readonly", width=30,
                                          font=("Inter", 11))
        self.category_menu.grid(row=0, column=1, sticky="w", padx=10, pady=10)

        # Exercise Input
        tk.Label(log_card, text="Exercise Name:", font=("Inter", 12, "bold"), bg=COLOR_CARD_BG, fg=COLOR_TEXT).grid(
            row=1, column=0, sticky="w", padx=10, pady=10)
        self.workout_entry = tk.Entry(log_card, width=30, font=("Inter", 11), bd=1, relief=tk.FLAT,
                                      highlightcolor=COLOR_PRIMARY, highlightthickness=1)
        self.workout_entry.grid(row=1, column=1, sticky="w", padx=10, pady=10)

        # Duration Input
        tk.Label(log_card, text="Duration (min):", font=("Inter", 12, "bold"), bg=COLOR_CARD_BG, fg=COLOR_TEXT).grid(
            row=2, column=0, sticky="w", padx=10, pady=10)
        self.duration_entry = tk.Entry(log_card, width=15, font=("Inter", 11), bd=1, relief=tk.FLAT,
                                       highlightcolor=COLOR_PRIMARY, highlightthickness=1)
        ...
        ttk.Button(button_frame, text="✅ ADD SESSION", command=self.add_workout, style="Primary.TButton",
                   width=18).grid(row=0, column=0, padx=15)
        ttk.Button(button_frame, text="📋 VIEW SUMMARY", command=self.view_summary, style="Secondary.TButton",
                   width=18).grid(row=0, column=1, padx=15)

        # Status Bar
        self.status_label = tk.Label(self.log_tab, text="Welcome! Ready for a great session.", bd=1, relief=tk.FLAT,
                                     anchor=tk.W, bg=COLOR_CARD_BG, fg="#6C757D", font=("Inter", 10))
    ...
    def view_summary(self):
        ...
        tk.Label(summary_window, text="🏋️ Full Session History", font=("Inter", 16, "bold"), bg=COLOR_CARD_BG,
                 fg=COLOR_TEXT).pack(pady=10)
        ...
        summary_text = tk.Text(text_frame, height=20, width=55, wrap=tk.WORD, font=("Inter", 10), bg=COLOR_BACKGROUND,
                               fg=COLOR_TEXT, yscrollcommand=scrollbar.set, relief=tk.FLAT)
        ...
        for category, sessions in self.workouts.items():
            ...
            summary_text.tag_config(category.lower(), font=("Inter", 12, "bold"),
                                    foreground=COLOR_SECONDARY if category == "Warm-up" else COLOR_PRIMARY if category == "Workout" else "#FFC107")
    ...
    def create_workout_plan_tab(self):
        tk.Label(self.chart_tab, text="💡 Personalized Workout Plan Guide", font=("Inter", 20, "bold"),
                 bg=COLOR_BACKGROUND, fg=COLOR_TEXT).pack(pady=20)

        plan_card = tk.Frame(self.chart_tab, bg=COLOR_CARD_BG, padx=30, pady=20, bd=0, relief=tk.FLAT,
                             highlightbackground="#E9ECEF", highlightthickness=1)
        plan_card.pack(pady=10, padx=50, fill="x")

        chart_data = {
            "Warm-up (5-10 min)": ["5 min light cardio (Jog/Cycle) to raise heart rate.",
                                   "Jumping Jacks (30 reps) for dynamic mobility.",
                                   "Arm Circles (15 Fwd/Bwd) to prepare shoulders."],
            "Strength & Cardio (45-60 min)": ["Push-ups (3 sets of 10-15) - Upper body strength.",
            ...
            "Cool-down (5 min)": ["Slow Walking - Bring heart rate down gradually.",
        ...
        for category, exercises in chart_data.items():
            tk.Label(plan_card, text=f"🔥 {category}", font=("Inter", 14, "bold"), bg=COLOR_CARD_BG,
                     fg=COLOR_SECONDARY).pack(anchor="w", padx=10, pady=(15, 5))
            for ex in exercises:
                tk.Label(plan_card, text=f"   • {ex}", font=("Inter", 11), bg=COLOR_CARD_BG, fg="#495057",
                         wraplength=700, justify=tk.LEFT).pack(anchor="w", padx=30)

    # ------------------ DIET GUIDE TAB ------------------ #
    def create_diet_guide_tab(self):
        tk.Label(self.diet_tab, text="🥗 Nutritional Goal Setting Guide", font=("Inter", 20, "bold"),
                 bg=COLOR_BACKGROUND, fg=COLOR_TEXT).pack(pady=20)

        diet_card = tk.Frame(self.diet_tab, bg=COLOR_CARD_BG, padx=30, pady=20, bd=0, relief=tk.FLAT,
                             highlightbackground="#E9ECEF", highlightthickness=1)
        diet_card.pack(pady=10, padx=50, fill="x")

        diet_plans = {
            "🎯 Weight Loss Focus (Calorie Deficit)": ["Breakfast: Oatmeal with Berries (High Fiber).",
            ...
            "🏃 Endurance Focus (Complex Carbs)": ["Pre-Workout: Banana & Peanut Butter (Quick Energy).",
        ...
        for goal, foods in diet_plans.items():
            tk.Label(diet_card, text=f"{goal}", font=("Inter", 14, "bold"), bg=COLOR_CARD_BG, fg=COLOR_PRIMARY).pack(
                anchor="w", padx=10, pady=(15, 5))
            for food in foods:
                tk.Label(diet_card, text=f"   • {food}", font=("Inter", 11), bg=COLOR_CARD_BG, fg="#495057",
                         wraplength=700, justify=tk.LEFT).pack(anchor="w", padx=30)

    # ------------------ PROGRESS TRACKER TAB ------------------ #
    def create_progress_tab(self):
        tk.Label(self.progress_tab, text="📈 Personal Progress Tracker", font=("Inter", 20, "bold"), bg=COLOR_CARD_BG,
                 fg=COLOR_TEXT).pack(pady=(20, 10))
        tk.Label(self.progress_tab, text="Visualization of your logged workout time distribution.", font=("Inter", 12),
                 bg=COLOR_CARD_BG, fg="#6C757D").pack(pady=(0, 20))
    ...
    def update_progress_charts(self):
        ...
        tk.Label(self.progress_tab, text=summary_text, font=("Inter", 13, "bold"), bg=COLOR_CARD_BG, fg="#DC3545").pack(
            pady=(10, 5))
